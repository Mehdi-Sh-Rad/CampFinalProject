    ...
    <div className={`border-b dark:bg-shop-dark dark:border-b-black border-b-slate-100 py-3 px-10 flex justify-between`}>
      ...
      <div className="w-1/2 flex gap-x-2 justify-end">
        <button className={`${isDarkMode ? "text-white" : "text-shop-dark"}`} onClick={toggleDarkMode}>
          <svg className={`text-gray-500 "w-6 h-6" ${isDarkMode && "hidden"}`} aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
        ...
        <div
          className={`absolute rounded top-16 bg-white z-20 left-4 overflow-hidden transition-all duration-300 py-2 px-4 shadow-sm  ${showMenu ? "flex items-center gap-x-2 opacity-100" : "hidden opacity-0"
            }`}>
