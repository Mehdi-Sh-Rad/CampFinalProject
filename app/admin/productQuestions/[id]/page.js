...
const AnswerProductQuestion = () => {
  ...
  useEffect(() => {
    const fetchProductQuestions = async () => {
      setLoading(true);
      try {
        const response = await fetch(`/api/productQuestions/${id}`);
  ...
  return (
    <div className="bg-shop-bg dark:bg-[#171a26] min-h-[100vh]">
      <div className="relative h-[180px] min-h-[180px] w-full overflow-hidden rounded-b-xl">
        <h1 className="text-white absolute z-10 right-8 top-6 font-bold text-xl md:text-3xl">ویرایش سوالات محصول </h1>
        <span className="text-white absolute z-10 right-8 top-20 text-xs sm:text-base">از این قسمت به سوالات پاسخ دهید و یا پاسخ خود را اصلاح کنید.</span>
        <Image
