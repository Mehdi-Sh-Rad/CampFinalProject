...
const UpdateTicket = () => {
  ...
  useEffect(() => {
    const fetchTicket = async () => {
      ...
      try {
        ...
        if (!response.ok) throw new Error(" مشکل در دریافت اطلاعات تیکت");
  ...
  const validateForm = () => {
    if (answer.trim() === "") {
      setFormError("درج پاسخ جدید الزامی میباشد");
      return false;
    } else if (answer.length < 1 || answer.length > 100) {
      setFormError("مشکل باید بین ۱ تا ۱۰۰ کاراکتر باشد");
      return false;
    }
    setFormError("");
    return true;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error("مشکلی در ساخت تیکت پیش آمده است");
  ...
  return (
    <div className="bg-shop-bg dark:bg-[#171a26] min-h-[100vh]">
      <div className="relative h-[180px] min-h-[180px] w-full overflow-hidden rounded-b-xl">
        <h1 className="text-white absolute z-10 right-8 top-6 font-bold text-xl md:text-3xl">ویرایش پاسخ تیکت </h1>
        <span className="text-white absolute z-10 right-8 top-20 text-xs sm:text-base">از این قسمت می توانید پاسخ تیکت را ویرایش کنید.</span>
        <Image
            ...
            <form className="py-4" onSubmit={handleSubmit}>
              <div className="flex flex-col items-start gap-y-4 w-full">
                ...
                <p className="focus:outline-none border dark:bg-shop-dark dark:border-gray-600 dark:text-gray-200! border-gray-200 rounded px-4 py-2 w-full transition-all duration-300">
                  {loading ? "در حال بارگذاری..." : ticket.topic || "not available"}</p>

                <label> شماره سفارش یا محصول مورد نظر</label>
                <p className="focus:outline-none border dark:bg-shop-dark dark:border-gray-600 dark:text-gray-200! border-gray-200 rounded px-4 py-2 w-full transition-all duration-300">
                  {loading ? "در حال بارگذاری..." : ticket.order || "not available"}</p>

                <label> مشکل </label>
                <p className="focus:outline-none border dark:bg-shop-dark dark:border-gray-600 dark:text-gray-200! border-gray-200 rounded px-4 py-2 w-full transition-all duration-300">
                  {loading ? "در حال بارگذاری..." : ticket.problem || "not available"}</p>

                <label> پاسخ </label>
                <input
                  name="answer"
                  className="focus:outline-none border dark:bg-shop-dark dark:border-gray-600 dark:text-gray-200 dark:placeholder:text-gray-200 border-gray-200 rounded px-4 py-2 w-full focus:ring-2 focus:ring-shop-red transition-all duration-500"
                  placeholder={`${loading ? "در حال بارگذاری..." : ticket.answer}`}
                  type="text"
                  value={answer}
